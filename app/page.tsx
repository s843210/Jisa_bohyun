...
const copy = {
  ...
  ja: {
    ...
    profileBody:
      "最も難しかったのは、慣れない日本語で会議の内容を正確に理解しながら、初めて取り組む仕事を期限内に完成させることでした。\nフィードバックを誤解して見当違いの箇所を修正したことも、動画の読み込みやホバー動作など、原因がすぐに分からない問題の前で長く立ち止まったこともあります。\nそれでも、分からない時はもう一度質問し、問題を一つずつ切り分け、毎週のフィードバックを次の成果物に反映しました。\nその経験から、失敗しないことよりも、最後まで理解し、改善し続ける姿勢の方が大切だと学びました。",
    placeLabel: "FIELD",
    placeValue: "US Medical Inc. · Tokyo",
    periodLabel: "PERIOD",
    ...
    packageBody:
      "最も難しかったのは、大人向けと子ども向けをそれぞれ制作したにもかかわらず、二つのパッケージが似た印象に見えるというフィードバックを受けたことでした。\n自分の目にはどちらも問題なく見え、何を基準に直せばよいのか迷いました。\nその後、子ども・保護者・一般の大人が最初に確認する情報を整理し直し、色・キャラクター・文言・情報量を分けました。\nその過程で、見た目の良さよりも、誰が見て選ぶのかを先に考えることの大切さを学びました。",
    kids: "KIDS / FAMILY",
...
const responsibilities: Array<{
...
}> = [
  {
    ...
    body: { ko: "장면마다 인물과 제품의 분위기가 달라 AI 영상 전체의 톤을 맞추기 어려웠고, 빠른 편집 속도 안에서 자막 가독성까지 지켜야 했습니다.", ja: "シーンごとに人物や製品の雰囲気が変わり、AI動画全体のトーンをそろえることが難しく、速い編集の中で日本語字幕の読みやすさも保つ必要がありました。" },
    tool: "AI FILM / EDIT",
  },
  {
    no: "02",
    title: { ko: "기업 홈페이지 영상 통합", ja: "企業サイトへの動画統合" },
    body: { ko: "고화질 영상을 넣자 로딩이 느려지고 모바일에서 화면이 잘렸습니다. 자동 재생 조건과 화질, 반응형 크기를 동시에 맞추는 것이 어려웠습니다.", ja: "高画質動画を入れると読み込みが遅くなり、モバイルでは画面が切れました。自動再生の条件、画質、レスポンシブサイズを同時に調整することが難しかったです。" },
    tool: "REACT / WEB",
  },
  {
    no: "03",
    title: { ko: "호버 재생 인터랙션", ja: "ホバー再生インタラクション" },
    body: { ko: "빠르게 마우스를 옮기면 여러 영상이 겹쳐 재생되거나 멈추지 않았습니다. 이벤트 순서와 상태 변화를 하나씩 추적해야 했습니다.", ja: "カーソルを素早く動かすと複数の動画が重なって再生され、停止しないこともありました。イベントの順序と状態変化を一つずつ追う必要がありました。" },
    tool: "INTERACTION / UI",
  },
  {
    no: "04",
    title: { ko: "관리자 화면·대시보드", ja: "管理画面・ダッシュボード" },
    body: { ko: "내게 익숙한 화면이 처음 쓰는 담당자에게는 복잡했습니다. 정보 우선순위와 수정 흐름을 사용자 관점에서 다시 정리하는 것이 어려웠습니다.", ja: "自分には分かりやすい画面でも、初めて使う担当者には複雑でした。情報の優先順位と編集の流れを利用者の視点で組み直すことが難しかったです。" },
    tool: "ADMIN / OPS",
  },
  {
    no: "05",
    title: { ko: "타깃별 제품 패키지", ja: "ターゲット別製品パッケージ" },
    body: { ko: "성인용과 어린이용을 따로 만들었지만 비슷해 보인다는 피드백을 받았습니다. 타깃별 분위기를 나누면서 필수 정보도 빠뜨리지 않아야 했습니다.", ja: "大人向けと子ども向けを分けて制作しても、似て見えるという指摘を受けました。対象ごとの印象を変えながら、必要な表示情報も欠かさないことが難しかったです。" },
    tool: "PACKAGE / PRINT",
  },
];
    ...
    <main id="top" data-lang={lang}>
      ...
      <section className="profile-section section-pad" id="profile">
        <div className="profile-intro" data-reveal>
          ...
          <p>{t.profileBody.split("\n").map((line, index) => <span key={`${lang}-${index}`}>{line}</span>)}</p>
        </div>
